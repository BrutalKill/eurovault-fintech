    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 480, background: 'hsl(240,26%,8%)', border: '1px solid hsl(240,16%,18%)', borderRadius: 20, padding: '40px 36px', boxShadow: '0 18px 48px rgba(0,0,0,0.5)', position: 'relative' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {[['Nome Completo','full_name','text','João Silva'],['Email','email','email','seu@email.com'],['Telefone','phone','tel','+351 912 345 678']].map(([label, key, type, placeholder]) => (
            <div key={key}>
        ...
        <div style={{ textAlign: 'center', marginTop: 20 }}>
          ...
          <Link to="/login" style={{ fontSize: 13, color: 'hsl(214,100%,60%)', fontWeight: 600, textDecoration: 'none' }}>Entrar</Link>
        </div>
