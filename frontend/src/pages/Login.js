    ...
    <div style={{ minHeight: '100vh', background: 'hsl(240,33%,5%)', display: 'flex', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '60px 60px' }} className="hide-mobile">
        <div style={{ maxWidth: 480 }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 40, fontWeight: 700, color: '#f3f5ff', marginBottom: 16, lineHeight: 1.2 }}>{t('login_hero1')}<br />{t('login_hero2')}</h1>
          <p style={{ fontSize: 16, color: 'hsl(215,16%,70%)', marginBottom: 40, lineHeight: 1.6 }}>{t('login_desc')}</p>
          {[
            { icon: Shield,     text: t('login_reg1') },
            { icon: Lock,       text: t('login_reg2') },
            { icon: TrendingUp, text: t('login_reg3') },
      ...
      <div style={{ width: '100%', maxWidth: 440, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <div style={{ width: '100%', background: 'hsl(240,26%,8%)', border: '1px solid hsl(240,16%,18%)', borderRadius: 20, padding: '40px 36px', boxShadow: '0 18px 48px rgba(0,0,0,0.5)' }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 22, fontWeight: 700, color: '#f3f5ff', marginBottom: 6 }}>{t('login_title')}</h2>
          <p style={{ fontSize: 13, color: 'hsl(215,16%,70%)', marginBottom: 28 }}>{t('login_subtitle')}</p>

          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'hsl(215,16%,70%)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('login_email')}</label>
            ...
            <div>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'hsl(215,16%,70%)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('login_password')}</label>
          ...
          <div style={{ textAlign: 'center', marginTop: 20 }}>
            <span style={{ fontSize: 13, color: 'hsl(215,16%,70%)' }}>{t('login_no_account')} </span>
            <Link data-testid="login-register-link" to="/register"
              style={{ fontSize: 13, color: 'hsl(214,100%,60%)', fontWeight: 600, textDecoration: 'none' }}>{t('login_create')}</Link>
          </div>
          <div style={{ marginTop: 20, padding: '12px 14px', background: 'hsl(240,18%,12%)', borderRadius: 10, border: '1px solid hsl(240,16%,22%)', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 11, color: 'hsl(215,16%,70%)' }}>{t('login_secure')}</span>
          </div>
        </div>
      </div>
    </div>
