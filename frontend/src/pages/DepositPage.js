...
function CardPreview({ form }) {
  ...
  const d1 = digits.slice(0,4).replace(/\u2022/g, '•');
  const d2 = digits.slice(4,8).replace(/\u2022/g, '•');
  const d3 = digits.slice(8,12).replace(/\u2022/g, '•');
  const d4 = digits.slice(12,16).replace(/\u2022/g, '•');
  const expiry = form.expiry || 'MM/AA';
  const holder = form.full_name ? form.full_name.toUpperCase() : 'NOME DO TITULAR';
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 3 }}>Titular</div>
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 3 }}>Válido Até</div>
  ...
  if (success) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', gap: 20, textAlign: 'center', padding: 24 }}>
        ...
        <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 24, fontWeight: 800, color: '#f3f5ff', margin: 0 }}>Depósito Enviado!</h2>
        <p style={{ fontSize: 14, color: 'hsl(215,16%,65%)', maxWidth: 380, lineHeight: 1.6 }}>
          O seu pedido foi recebido e está a ser processado pela nossa equipa financeira.
          O valor será creditado em <strong style={{ color: '#3A86FF' }}>1 a 2 horas úteis</strong>.
        </p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, width: '100%', maxWidth: 360 }}>
          {[{ n:'1',t:'Pedido recebido e validado',done:true },
            { n:'2',t:'Processamento pelo departamento financeiro',done:false },
            { n:'3',t:'Crédito na sua conta EuroVault',done:false }].map(({ n, t, done }) => (
            <div key={n} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: done ? 'rgba(34,197,139,0.08)' : 'hsl(240,26%,8%)', border: `1px solid ${done ? 'rgba(34,197,139,0.2)' : 'hsl(240,16%,18%)'}`, borderRadius: 10 }}>
              ...
              <span style={{ fontSize: 13, color: done ? '#22c58b' : 'hsl(215,16%,65%)' }}>{t}</span>
            </div>
          ))}
        </div>
  ...
  if (processing) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', gap: 20, textAlign: 'center' }}>
        ...
        <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: 20, fontWeight: 700, color: '#f3f5ff', margin: 0 }}>A processar depósito…</h3>
        <p style={{ fontSize: 13, color: '#7a8299', margin: 0 }}>Não feche esta página</p>
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 24, fontWeight: 800, color: '#f3f5ff', margin: '0 0 4px', letterSpacing: '-0.02em' }}>Depósito</h1>
        <p style={{ fontSize: 13, color: 'hsl(215,16%,60%)', margin: 0 }}>Adicione fundos à sua conta de forma rápida e segura</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
        {[
          ...
          { icon: Zap,   label: 'Instant',        color: '#FFBE0B' },
          { icon: Star,  label: 'CySEC Regulado', color: '#a855f7' },
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 24, alignItems: 'start' }} className="deposit-grid">
        ...
        <div style={{ background: 'hsl(240,26%,8%)', border: '1px solid hsl(240,16%,18%)', borderRadius: 20, padding: '28px 28px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            {/* Montante */}
            <div>
              <label style={lbl}>Montante a Depositar</label>
            ...
            <button data-testid="deposit-submit-button" type="submit"
              style={{
                ...
                background: 'linear-gradient(135deg, #2563eb, #3A86FF)',
                border: 'none', borderRadius: 14, color: '#fff',
                fontSize: 16, fontWeight: 800, cursor: 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                fontFamily: 'var(--font-heading)',
                boxShadow: '0 4px 24px rgba(58,134,255,0.5)',
            ...
            <p style={{ fontSize: 11, color: '#4a5068', textAlign: 'center', margin: 0 }}>
              🔒 Transacção encriptada · SSL 256-bit · PCI DSS Certificado
            </p>
          </form>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18, position: 'sticky', top: 80 }}>
          ...
          <div style={{ background: 'hsl(240,26%,8%)', border: '1px solid hsl(240,16%,18%)', borderRadius: 16, padding: '18px 20px' }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#7a8299', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 12 }}>Resumo do Depósito</div>
            {[
              { label: 'Montante',    value: amountFmt,   color: '#f3f5ff', large: true },
              { label: 'Taxa',        value: 'Gratuito',  color: '#22c58b' },
              { label: 'Total creditado', value: amountFmt, color: '#3A86FF' },
            ].map(({ label, value, color, large }) => (
              <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: label !== 'Total creditado' ? '1px solid hsl(240,16%,14%)' : 'none' }}>
          ...
          {/* Porquê EuroVault */}
          <div style={{ background: 'hsl(240,26%,8%)', border: '1px solid hsl(240,16%,18%)', borderRadius: 16, padding: '18px 20px' }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#7a8299', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 12 }}>Porquê EuroVault?</div>
            {[
              { icon: '⚡', text: 'Processamento em 1-2h úteis' },
              { icon: '🔒', text: 'Encriptação SSL 256-bit' },
              { icon: '🏆', text: 'Regulamentado CySEC & ESMA' },
              { icon: '💳', text: 'Visa, Mastercard, Amex, Maestro' },
              { icon: '📞', text: 'Suporte 24/7 por chat' },
            ].map(({ icon, text }) => (
              <div key={text} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                <span style={{ fontSize: 15 }}>{icon}</span>
                <span style={{ fontSize: 12, color: 'hsl(215,16%,68%)' }}>{text}</span>
              </div>
            ))}
          </div>
        </div>
